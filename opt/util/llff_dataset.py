...
class LLFFDataset(DatasetBase):
    ...
    def stylized_gt_update(self, stylized_imgs):
        self.gt = torch.stack(stylized_imgs).float()
        self.n_images, self.h_full, self.w_full, _ = self.gt.shape
        assert self.h_full == self.sfm.ref_cam["height"]
        assert self.w_full == self.sfm.ref_cam["width"]
        ...
        self.ndc_coeffs = (2 * self.intrins_full.fx / self.w_full,
                           2 * self.intrins_full.fy / self.h_full)
        # JULIAN: also update rays here: all following steps
        if self.split == "train":
            self.gen_rays(factor=self.factor)
        else:
            # Rays are not needed for testing
            self.h, self.w = self.h_full, self.w_full
            self.intrins = self.intrins_full
    ...
    def replace_render_data(self, stylized_imgs, val_c2w=None):
        # JULIAN: update the c2w by using a dictionary.
        if val_c2w is not None:
            self.c2w = val_c2w
        self.gt = torch.stack(stylized_imgs).float() / 255.0
        self.n_images, self.h_full, self.w_full, _ = self.gt.shape
        assert self.h_full == self.sfm.ref_cam["height"]
        assert self.w_full == self.sfm.ref_cam["width"]
        ...
        self.ndc_coeffs = (2 * self.intrins_full.fx / self.w_full,
                           2 * self.intrins_full.fy / self.h_full)
        # JULIAN: default to generate rays.
        self.gen_rays(factor=self.factor)
        self.should_use_background = False  # Give warning
